...
.controller('DashCtrl', function($scope, $http, $localstorage,TDCardDelegate,$timeout) {
 ...
 //  if(Object.keys($localstorage.getObject('temp')).length ===0){
      ...
      $http.get("http://restfulrecipes.azurewebsites.net/explore").success(function(data, status, headers, config) {
    ...
    /*} else{ //can also pull from local storage data to avoid overusing api
      ...
      $scope.cards = Array.prototype.splice.call(cardTypes, 1,2);
      $http.get("http://mealmatch.azurewebsites.net/start").success(function(data, status, headers, config) {
...
.controller('CardCtrl', function($scope, $http, TDCardDelegate) {
  ...
  $scope.cardSwipedRight = function(card) {
    ...
    console.log(card);
    // $scope.addCard();
    $http.post("http://restfulrecipes.azurewebsites.net/explore", card).success(function(data, status, headers, config) {
